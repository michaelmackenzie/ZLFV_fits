...
int ZLL_fit_v6(TString name="test", TString lepton="mu", TString data_file="bdt_tree_v2_data_mumu_part*.root", bool save_shape=true, int nbin=500, bool save_data_histo=true, TString directly_input_histo_file="None",TString input_histo="None"){
   
   TString dilep_var_name="mass_ll";
   TString cuts = "mass_ll>70 && mass_ll<110 && xgb>0.65";
   TString tree_name="mytreefit";
   TString electron_pdf="Zee_mc_pdf2018.root";
   ...
   if (directly_input_histo_file=="None"){
      ...
      if (save_data_histo){
        TFile * fdata_histo = new TFile("fdata_from_fit_Z"+leplep+".root","RECREATE");
        hdata->Write();
        cout<<"====> saved in fdata_from_fit_Z"+leplep+".root"<<endl;
      }
   } else{
   ...
   RooRealVar dilep_mass("mass_ll","m("+label+")", 110., min_fit_range , max_fit_range, "GeV/c^{2}");
   ...
   dilep_mass.setBins(nbin*30);

   RooDataHist datah("datah","",RooArgSet(dilep_mass),hdata);
   RooRealVar bkg_cmsshape_a("bkg_cmsshape_a", "", 60., 50., 190.); // 60., 50., 190.
   RooRealVar bkg_cmsshape_b("bkg_cmsshape_b", "", 0.05, 0.01, 0.10); // 0.05, 0.01, 0.12
   RooRealVar bkg_cmsshape_c("bkg_cmsshape_c", "", 0.1, -2., 2.);
   RooRealVar bkg_cmsshape_peak("bkg_cmsshape_peak", "", 91.0); 

   RooCMSShape bkg_cmsshape("bkg_cmsshape", "", dilep_mass,bkg_cmsshape_a,bkg_cmsshape_b,bkg_cmsshape_c,bkg_cmsshape_peak);

   TFile * fpdf = new TFile(electron_pdf,"READ");
   TH1F* hpdf = (TH1F*) fpdf->Get("hdata");
   RooDataHist hsgn_pdf("hsgn_pdf","",RooArgSet(dilep_mass),hpdf);
   RooHistPdf sgn_pdf("sgn_pdf", "", dilep_mass, hsgn_pdf);
   RooRealVar sgn_gauss_mean("sgn_gauss_mean", "", 0.0, -5.0, 5.0); // 60., 50., 190.
   RooRealVar sgn_gauss_width("sgn_gauss_width", "", 0.9, 0.05, 5.0); // 0.05, 0.01, 0.12

   RooGaussian sgn_cb("sgn_cb","",dilep_mass,sgn_gauss_mean,sgn_gauss_width);
   RooFFTConvPdf sgn_PDF("sgn_PDF","",dilep_mass,sgn_pdf,sgn_cb) ;
   ...
   RooFitResult * result = total_PDF.fitTo(datah,RooFit::Extended(1),RooFit::Save(),RooFit::Range("fitRange"),RooFit::Minos(true)); //RooFit::PrintLevel(-1)
   ...
   save_plot(frame,"m("+label+")","v5_cr_"+leplep+"_"+name,true,false,"Z->"+label,TString(std::to_string(int(nsgn.getVal()))), TString(std::to_string(int(nbkg.getVal()))));
   save_pull(frame, dilep_mass, "m(l,l)","v5_cr_"+leplep+"_"+name);
