...
samples = ["/eos/cms/store/cmst3/user/gkaratha/ZmuE_forBDT_v7_tuples/BDT_outputs_v7/NewElectronID/Meas_fullAndSFAndGenDecay_bdt_v7_bkg_dy_mcRun1*.root",\
"/eos/cms/store/cmst3/user/gkaratha/ZmuE_forBDT_v7_tuples/BDT_outputs_v7/NewElectronID/Meas_fullAndGenWTonly_bdt_v7_bkg_ww_mcRun1*_updateID.root",\
"/eos/cms/store/cmst3/user/gkaratha/ZmuE_forBDT_v7_tuples/BDT_outputs_v7/NewElectronID/Meas_fullAndGenWTonly_bdt_v7_bkg_ttbar_mcRun1*_updateID.root",\
"/eos/cms/store/cmst3/user/gkaratha/ZmuE_forBDT_v7_tuples/BDT_outputs_v7/NewElectronID/Meas_fullAndSF_bdt_v7_signal_mcRun1*.root"]
...
data_sample_sgn_weight = "/eos/cms/store/cmst3/user/gkaratha/ZmuE_forBDT_v7_tuples/BDT_outputs_v7/Meas_full_bdt_v7_data_emu_Run1*.root"
...
datatree = rt.TChain("mytreefit","mytreefit")
datatree.Add(data_sample_sgn_weight)
...
outtree.Branch("NormGen_wt",NormGen_wt,"NormGen_wt/F")
...
for ismp in range(len(samples)):
   ...
   print "sample",sample_names[ismp]
   print "weighted num",nums[ismp],"den",dens[ismp]
   evt_max=intree.GetEntries()
   norm_wt=nums[ismp]/dens[ismp]*Br_xsecs[ismp]*lumi/evt_max
   if signal_index==ismp:
      norm_wt=nums[ismp]/dens[ismp]*Br_xsecs[ismp]*lumi/evt_max*(bkg_weighted_evts/datatree.GetEntries())
      norm_wt*=signal_r
   print "will run on",evt_max,"normalized to",norm_wt*evt_max
   ...
   for evt in intree:
      ...
      sources[ismp][0]=1
      mass[0]=evt.mass_ll
      NormGen_wt[0] = norm_wt*SignGen_wt[0]
      if dy_index==ismp and evt.IsGen_Zmm:
         NormGen_wt[0] = norm_wt*evt.SignGen_wt*zmm_dy_r
      if signal_index!=ismp:
         bkg_weighted_evts+=NormGen_wt[0]
...
name="pseudo_data_from_MC_v2_r"+str(signal_r)
if zmm_dy_r!=1:
  name+="_ZmmR"+str(zmm_dy_r)
fout = rt.TFile(name+"_updateID.root","RECREATE")
print name+".root"
outtree.Write()
