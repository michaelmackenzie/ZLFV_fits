...
int ZMuE_fit_mk2_Zmm_v1(TString name="bin1_r2", 
   ...
   RooDataHist * dhist_zmm = new RooDataHist("dhist_zmm_"+varname,"dhist_zmm_"+varname,RooArgSet(dilep_mass),hzmm);
   ...
   if (Fix_mean)
      ...
      zmm_dcb_mean->setRange(zmm_dcb_mean->getVal()*0.95,zmm_dcb_mean->getVal()*1.02);
   ...
   if (create_dc_input){
     ...
     RooDoubleCrystalBall zmm_dcb_pdf_out("zmm_dcb_pdf_"+varname, "zmm_dcb_pdf_"+varname, dilep_mass_out, *zmm_dcb_mean, *zmm_dcb_sigma, *zmm_dcb_a1, *zmm_dcb_n1, *zmm_dcb_a2, *zmm_dcb_n2);
