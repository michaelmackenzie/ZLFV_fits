...
parser.add_argument("--obs", dest="obs",action='store_true', default=False,help="Run on the observed data instead of Asimov")
...
groups = {
    "envelope" : ["pdfindex_bin1", "pdfindex_bin2", "pdfindex_bin3"], # Effect is only visible in observed fits
    "params" : [],
    "IDs_trig" : ["idEleSF", "idMuSF", "isoEleSF", "isoMuSF", "dxyEleSF", "dxyMuSF", "recoEleSF", "triggersSF"],
    "mu_scale" : ["nuisance_mu_scale_bin1", "nuisance_mu_scale_bin2", "nuisance_mu_scale_bin3"],
    "ele_scale" : ["nuisance_ele_scale_bin1", "nuisance_ele_scale_bin2", "nuisance_ele_scale_bin3"],
    "stats" : ["statsBin1", "statsBin2", "statsBin3"],
    "zmumu_yield" : ["Zmm"],
    "jer_jes" : ["jer","jes"],
    "scales" : ["bdt", "btagSF", "puSF", "prefire", "mixSF", "xsec", "zptSF", "pdf", "lumi"],
    "btag" : ["btagSF"],
    "PU" : ["puSF"],
    "theory" : ["pdf","mixSF"],
    "lumi" : ["lumi"],
...
obs_line = "" if args.obs else "-t -1"
if not args.skip_fits:
    print ">>> Processing the nominal fit"
    command = base_dir + '/tests/zemu_fit.sh ' + args.card + ' "' + obs_line + '" ' + args.tag + '_nominal ' + args.rrange
    os.system(command)
...
for group in groups:
    params = groups[group]
    if not args.skip_fits:
        command = base_dir + '/tests/zemu_fit.sh ' + args.card + ' "' + obs_line + ' --freezeParameters '
        if group == "params":
