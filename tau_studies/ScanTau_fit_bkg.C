...
int ScanTau_fit_bkg(TString name="bin1_r2",
    ...
    float xgbmin=0.7,float xgbmax=1., double  min_fit_range=90,
    double  max_fit_range = 135, double blind_min=106, double blind_max=114,
    bool unblind=false, bool create_dc_input=false, TString outvar="mcol",
    TString varname="bin",TString outdir="tau_workspaces/"){
   ...
   /// generic
   TString cuts = "Flag_met && Flag_muon && Flag_electron && Flag_tau";
   if(xgbmin < xgbmax) cuts += Form(" && xgb > %.2f && xgb <= %.2f", xgbmin, xgbmax);
   ...
   // read trees
   RooRealVar dilep_mass ("mcol","m(l,#tau)", min_fit_range, min_fit_range , max_fit_range, "GeV/c^{2}");
   RooRealVar dilep_mass_out(outvar,"m(l,#tau)", min_fit_range, min_fit_range , max_fit_range, "GeV/c^{2}");
   ...
   TChain * cc = new TChain("mytree");
   cc->Add(data_file);
   cc->Draw("mcol>>hbkg",cuts);
   ...
   wspace->writeToFile(outdir+"workspace_bkg_"+name+".root"); // write the output
