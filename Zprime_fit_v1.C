...
int Zprime_fit_v1(TString name="test",  TString data_file="",float sgn_mass=125., TString xgbmin="0.75",TString xgbmax="1.01",  bool unblind=false, bool create_dc_input=false, float expected_Nsgn=350, TString outvar="mass_ll", bool syst_sgn=true, bool altfit_bkg=true, TString varname="bin"){

   gROOT->SetBatch(true);
   data_file="/eos/cms/store/cmst3/user/gkaratha/ZmuE_forBDT_v7_tuples/BDT_outputs_v7/Scan_Zprime/Meas_full_bdt_v7_zprime_data_emu_Run1*.root";
   TString sgn_file_m65="/eos/cms/store/cmst3/user/gkaratha/ZmuE_forBDT_v7_tuples/BDT_outputs_v7/Scan_Zprime/Meas_fullAndSF_bdt_v7_zprimeM65_emu_mcRun18.root";
   TString sgn_file_m91="/eos/cms/store/cmst3/user/gkaratha/ZmuE_forBDT_v7_tuples/BDT_outputs_v7/Meas_fullAndSF_bdt_v7_signal_mcRun18.root";
   TString sgn_file_m150="/eos/cms/store/cmst3/user/gkaratha/ZmuE_forBDT_v7_tuples/BDT_outputs_v7/Scan_Zprime/Meas_fullAndSF_bdt_v7_zprimeM150_emu_mcRun18.root";
   ...
   double  min_fit_range = sgn_mass-25;
   double  max_fit_range = sgn_mass+25;
   double blind_min = sgn_mass-5;
   double blind_max = sgn_mass+5;
   int nbin_data = 80;
   bool Verbose=false;
...
////////////////////////////////////////////////////////////////////////////////
   ...
   // read trees
   TTree * sgn_tree_m65 = get_tree("mytreefit",sgn_file_m65,cuts);
   TTree * sgn_tree_m91 = get_tree("mytreefit",sgn_file_m91,cuts);
   TTree * sgn_tree_m150 = get_tree("mytreefit",sgn_file_m150,cuts);
   std::vector<TTree*> sgn_trees={sgn_tree_m65,sgn_tree_m91,sgn_tree_m150};
   ...
   //////////////////////// signal parameters vs mass //////////////////////// 
   RooWorkspace ws ("ws");
   ws.import(dilep_mass);
   RooRealVar sgn_cb_means[3];
   RooRealVar sgn_cb_widths[3];
   RooRealVar sgn_cb_a1s[3];
   RooRealVar sgn_cb_n1s[3];
   RooRealVar sgn_cb_a2s[3];
   RooRealVar sgn_cb_n2s[3];
   double sgn_masses[]={65,91,150};

   for( int itree=0; itree< sgn_trees.size(); itree++){
     RooRealVar dilep_mass_temp("mass_ll","m(e,#mu)", sgn_masses[itree],sgn_masses[itree]-30, sgn_masses[itree]+30, "GeV/c^{2}");
     RooDataSet sgn_dataset("sgn_dataset","sgn_dataset",RooArgSet(dilep_mass_temp),RooFit::Import(*sgn_trees[itree]));
     sgn_cb_means[itree]=RooRealVar("sgn_cb_mean_"+varname+"_"+TString(std::to_string(itree)),"",sgn_masses[itree], sgn_masses[itree]-5, sgn_masses[itree]+5);
     sgn_cb_widths[itree]=RooRealVar("sgn_cb_width_"+varname+"_"+TString(std::to_string(itree)),"",4., 1., 10.);
     sgn_cb_a1s[itree]=RooRealVar("sgn_cb_a1_"+varname+"_"+TString(std::to_string(itree)),"",1.0, 0.1, 100.0);
     sgn_cb_n1s[itree]=RooRealVar("sgn_cb_n1_"+varname+"_"+TString(std::to_string(itree)),"",1.0, 0.00001, 100.0);
     sgn_cb_a2s[itree]=RooRealVar("sgn_cb_a2_"+varname+"_"+TString(std::to_string(itree)),"",1.0, 0.1, 100.0);
     sgn_cb_n2s[itree]=RooRealVar("sgn_cb_n2_"+varname+"_"+TString(std::to_string(itree)),"",1.0, 0.1, 100.0);
  
     RooDoubleCB sgn_PDF("sgn_PDF_temp","cb",dilep_mass_temp,sgn_cb_means[itree],sgn_cb_widths[itree],sgn_cb_a1s[itree],sgn_cb_n1s[itree],sgn_cb_a2s[itree],sgn_cb_n2s[itree]);
  
     RooRealVar n_sgn("n_sgn", "n_sgn",1000,0,10000000000);
     RooAddPdf esgn_PDF("sgn","esgnPDF",RooArgList(sgn_PDF),RooArgList(n_sgn));
     RooFitResult * sgn_result = esgn_PDF.fitTo(sgn_dataset,RooFit::Extended(1),RooFit::Save(),RooFit::Range(sgn_masses[itree]-30,sgn_masses[itree]+30),RooFit::PrintLevel(-1));
     auto sgn_frame = dilep_mass_temp.frame();   
     ...
     save_plot(sgn_frame,"m(#mu,e)","v1_prime_sgnVm_massponit_m"+TString(std::to_string(itree))+"_"+name);
   }
   double width_pars[2];
   int i=0;
   for (auto sgn_params: { sgn_cb_means, sgn_cb_widths, sgn_cb_a1s,
                           sgn_cb_n1s, sgn_cb_a2s, sgn_cb_n2s}){
     double sgn_params2[]={sgn_params[0].getVal(),sgn_params[1].getVal(),sgn_params[2].getVal()};
     double sgn_errors2[]={sgn_params[0].getError(),sgn_params[1].getError(),sgn_params[2].getError()};
     double sgn_merrors[]={0,0,0};
     TGraphErrors * gr = new TGraphErrors(3,sgn_masses,sgn_params2,sgn_merrors, sgn_errors2);     
     TCanvas * c1= new TCanvas("c1_"+TString(std::to_string(i)),"c1",700,700);
     gr->Draw("PAE"); 
     if (i==1){
       TF1* fnc = new TF1("fnc","pol1",63,152);
       gr->Fit("fnc","R");
       fnc->GetParameters(&width_pars[0]);
     }
     c1->SaveAs("fit_v1_prime_sgn_param_"+TString(std::to_string(i))+"_vs_mass_"+name+".png");
     i++;
   }
    

    ///////////////////// create signal peak for sgn_mass /////////////////////
    RooRealVar sgn_cb_mean=RooRealVar("sgn_cb_mean_"+varname,"",sgn_mass, sgn_mass-5, sgn_mass+5);
    RooRealVar sgn_cb_width=RooRealVar("sgn_cb_width_"+varname,"",sgn_mass*width_pars[1]+width_pars[0], 0.1, 10.);
    RooRealVar sgn_cb_a1=RooRealVar("sgn_cb_a1_"+varname,"",0.82, 0.1, 100.0);
    RooRealVar sgn_cb_n1=RooRealVar("sgn_cb_n1_"+varname,"",6.0, 0.00001, 100.0);
    RooRealVar sgn_cb_a2=RooRealVar("sgn_cb_a2_"+varname,"",1.05, 0.1, 100.0);
    RooRealVar sgn_cb_n2=RooRealVar("sgn_cb_n2_"+varname,"",5.8, 0.1, 100.0);
    RooDoubleCB sgn_PDF("sgn_PDF","cb",dilep_mass,sgn_cb_mean,sgn_cb_width,sgn_cb_a1,sgn_cb_n1,sgn_cb_a2,sgn_cb_n2);
    RooRealVar n_sgn("n_sgn", "n_sgn",expected_Nsgn,0,10000000000);
    RooAddPdf esgn_PDF("sgn","esgnPDF",RooArgList(sgn_PDF),RooArgList(n_sgn));      auto sgn_frame = dilep_mass.frame();
    esgn_PDF.plotOn(sgn_frame,RooFit::LineColor(kBlue));
   
    save_plot(sgn_frame,"m(#mu,e)","v1_prime_sgn_"+name);
   ...
   save_pull(xframe, dilep_mass, "m(e,#mu)","v1_prime_cheb3_"+name);
   ...
   save_plot(xframe,"m(e,#mu)","v1_prime_bkg_"+name,leg);


   /////////////////////////////  create combine input ////////////////////////
   if (!create_dc_input) return 0;
   //sgn
   ...
   wspace_sgn->writeToFile("workspace_v1_prime_sgn_"+name+".root");
   ...
   save_plot(gen_frame,"m(e,#mu)","v1_prime_gentest_"+name);
   ...
   bin_frm->Draw();
   csim->SaveAs("v1_prime_datasim_"+name+".png");
   wspace_bkg->import(*data_obs);
   wspace_bkg->import(hdata_sim);
   wspace_bkg->writeToFile("workspace_v1_prime_bkg_"+name+".root");
