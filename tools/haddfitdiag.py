...
for e in fileHandles[0].GetListOfKeys() :
	name=e.GetName()
        if name == "ParameterSets": #don't need this data, and can be large for some datasets
                ...
                continue
	print "Merging" ,name
        if not hasattr(e,'ReadObj'):
                print "Not a readable object!"
                continue
	obj=e.ReadObj()
	...
	for fh in fileHandles[1:] :
                if not hasattr(fh.GetListOfKeys().FindObject(name),'ReadObj'):
                        print "Not a readable object!"
                        continue
		otherObj=fh.GetListOfKeys().FindObject(name).ReadObj()
		inputs.Add(otherObj)
		if isTree and obj.GetName() in ['Events', 'Norm', 'mutau', 'etau', 'emu', 'mumu', 'ee']  :	
